...
from questionLogin import startQuestionLoginSecond
from registrationWindow import startRegistrationWindow


def startBaseLoginSecond(firstUser, firstLanguage, language):
    # Initialize pygame
    pygame.init()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if nextButtonRect.collidepoint(event.pos):
                    ...
                    try:
                        if questionCounter > 1:
                            pass
                            startQuestionLoginSecond(textUsername, language)
                            ...
                            setVariables(firstUser,textUsername,firstLanguage,1)
                ...
                if backButtonRect.collidepoint(event.pos):
                    buttonSoundEffect()
                    try:
                        startGame2(firstUser,firstLanguage)
                    except():
                        errorMessage = translateText("Error no reconocido, vuelva a intentarlo", target_language)
                        tkMessageBox.showerror("Error", errorMessage)
...
def startGame2(firstUser, firstLanguage):
    pygame.init()
